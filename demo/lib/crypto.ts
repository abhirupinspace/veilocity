...
// In production, this would use the Noir prover with Barretenberg
export function generateMockProof(
  ...
  const stateRoot = keccak256(
    encodePacked(["bytes32", "string"], [secret, "state_root"])
  );

  // Generate a mock proof (256 bytes of deterministic data)
  // Real proofs are generated by Barretenberg
  const proofSeed = keccak256(
    encodePacked(["bytes32", "uint256"], [secret, amountWei])
  );

  // Expand to 256 bytes by hashing repeatedly
  let proofBytes = proofSeed;
  for (let i = 0; i < 7; i++) {
    const nextChunk = keccak256(
      encodePacked(["bytes32", "uint256"], [proofBytes, BigInt(i)])
    );
    proofBytes = (proofBytes + nextChunk.slice(2)) as `0x${string}`;
  }

  return {
    nullifier,
    stateRoot,
    proofBytes: proofBytes.slice(0, 514) as `0x${string}`, // 256 bytes = 512 hex chars + 0x
  };
}
